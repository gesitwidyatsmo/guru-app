	...
	useEffect(() => {
		const fetchAll = async () => {
			try {
				...
				const { data: kelas } = await supabase.from('kelas').select('*').eq('id', classId).single();
				if (kelas) {
					setSelectedKelas(kelas.nama_kelas);
					setKelasList([kelas]);
				}

				const [dataMapel, dataSiswa] = await Promise.all([
						...
						<div className='neo-card bg-white p-4'>
							...
							<div className='relative'>
								<input
									...
									value={selectedKelas}
									readOnly
									className='neo-input bg-[#E8E8E8] text-[#0D0D0D] cursor-not-allowed w-full font-bold border-2 border-[#0D0D0D] shadow-[4px_4px_0px_0px_#0D0D0D]'
								/>
							</div>
						</div>
						<div className='neo-card bg-white p-4'>
